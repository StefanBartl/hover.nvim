...
--- `hover.preview.binary` first, and hands anything that is not text
...
--- through `hover.registry`, which is also why nothing in this file
--- requires it.
...
function M.file(target, opts)
  ...
  return {
    ...
    title = title,
    -- Consumed by `hover`'s scroll bindings. `more` is what decides
...
--- `HoverMissing` (linked to `DiagnosticError` by default, so it
...
function M.missing(target)
  ...
  return { lines = lines, title = "broken link", highlight = "HoverMissing" }
end
