...
---@field persist? boolean # Default true. Write `mode`, `auto_hover` and every switch back over this configuration on the next `enable()`, so a runtime change (`:Hover dashboard`, `:Hover links web on`, ...) outlives the session it was made in. `false` for a session-only override instead. See `hover.persist`.
